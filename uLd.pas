...
type
  ...
  TLd = class
  strict private
    class function GetRunConsoleResult(FileName: string; const Visibility: Integer; var mOutputs: string): Integer;
  public
    class var
      FilePath: string;
    class var
    ...
    class function GetProp(const index: Integer; key: string = ''; value: string = ''): string;
    ...
    class procedure DownCpu(const index: Integer; const rate: Integer { 0~10 } ); // 降低cpu
    ...
    // 修改定位即时生效
    class procedure LocateByAction(const index: Integer; const Lng, Lat: Integer);
    // 重命名
    class procedure Rename(const index: Integer; title: string);
    // 重启模拟器，启动后并打开 packagename 应用, null 表示不打开任何应用
    class procedure RebootByAction(const index: Integer; PackageName: string = 'null');
    // 摇一摇
    ...
    class procedure NetworkByAction(const index: Integer; const IsConnect: Boolean);
    class procedure Scan(const index: Integer; FileName: string);
    // 备份应用
    class procedure BackupApp(const index: Integer; PackageName: string; FileName: string);
    // 还原应用
    class procedure RestoreApp(const index: Integer; PackageName: string; FileName: string);
    // 模拟器是否运行
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s action --index %d --key %s --value %s', [FilePath, index, key, value]))), Visibility);
end;

class function TLd.Adb(const index: Integer; command: string): string;
begin
  GetRunConsoleResult(PAnsiChar(Ansistring(Format('%s adb --index %d --command "%s"', [FilePath, index, command]))), Visibility, Result);
end;

class procedure TLd.Add(name: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s add --name %s ', [FilePath, name]))), Visibility);
end;

class procedure TLd.Backup(const index: Integer; FileName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s backup --index %d --file %s', [FilePath, index, FileName]))), Visibility);
end;

class procedure TLd.BackupApp(const index: Integer; PackageName, FileName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s backup --index %d --packagename %s --file %s', [FilePath, index, PackageName, FileName]))), Visibility);
end;

class procedure TLd.Copy(name: string; const FromIndex: Integer);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s add --name %s --from %d ', [FilePath, name, FromIndex]))), Visibility);
end;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s downcpu --index %d --rate %d', [FilePath, index, rate]))), Visibility);
end;
...
begin
  ...
  begin
    commandLine := Format('%s getprop  --index 0 --key "%s" --value "%s"', [FilePath, key, value])
  end;
  GetRunConsoleResult(commandLine, TLd.Visibility, Result);
end;

class function TLd.GetRunConsoleResult(FileName: string; const Visibility: Integer; var mOutputs: string): Integer;
var
...
class procedure TLd.GlobalSetting(const fps, audio, fastply, cleanmode: Integer);
var
  comm: string;
begin
  comm := Format('%s --fps %d --audio %d  --fastplay %d --cleanmpde %d', [FilePath, fps, audio, fastply, cleanmode]);
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s installapp --index %d --filename %s', [FilePath, index, apkFileName]))), Visibility);
end;
...
begin
  Result := False;
  GetRunConsoleResult(Format('%s isrunning --index %d', [FilePath, index]), TLd.Visibility, r);
  if r = 'runing' then
    Result := True
  else if r = 'stop' then
    Result := False;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s killapp --index %d --packagename %s', [FilePath, index, PackageName]))), Visibility);

end;

class procedure TLd.Launch(const index: Integer);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s launch --index %d', [FilePath, index]))), Visibility);
end;

class procedure TLd.LaunchEx(const index: Integer; PackageName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s launchex --index %d --packagename "%s"', [FilePath, index, PackageName]))), Visibility);
end;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s locate --index %d --LLI %d,%d', [FilePath, index, Lng, Lat]))), Visibility);
end;
...
class procedure TLd.Modify(const index, w, h, dpi, cpu, memory: Integer; manufacturer, model, pnumber, imei, imsi, simserial, androidid, mac, autorotate, lockwindow: string);
var
  commandLine: string;
begin
...
class procedure TLd.NetworkByAction(const index: Integer; const IsConnect: Boolean);
var
  value: string;
begin
  if IsConnect then
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s quit --index %d', [FilePath, index]))), Visibility);
end;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s reboot --index %d', [FilePath, index]))), Visibility);
end;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s remove --index %d', [FilePath, index]))), Visibility);
end;

class procedure TLd.Rename(const index: Integer; title: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s rename --index %d --title %s', [FilePath, index, title]))), Visibility);
end;

class procedure TLd.Restore(const index: Integer; FileName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s restore --index %d --file %s', [FilePath, index, FileName]))), Visibility);
end;

class procedure TLd.RestoreApp(const index: Integer; PackageName, FileName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s restoreapp --index %d --packagename %s --file %s', [FilePath, index, PackageName, FileName]))), Visibility);
end;

class procedure TLd.RunApp(const index: Integer; PackageName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s runapp --index %d --packagename %s', [FilePath, index, PackageName]))), Visibility);
end;

class procedure TLd.Scan(const index: Integer; FileName: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s scan --index %d --file %s', [FilePath, index, FileName]))), Visibility);
end;

class procedure TLd.SetProp(const index: Integer; key, value: string);
begin
  WinExec(PAnsiChar(Ansistring(Format('%s setprop  --index %d --key "%s" --value "%s"', [FilePath, index, key, value]))), Visibility);
end;
...
begin
  WinExec(PAnsiChar(Ansistring(Format('%s uninstallapp --index %d --packagename %s', [FilePath, index, PackageName]))), Visibility);
end;
